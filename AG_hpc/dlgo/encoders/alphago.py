...
# the last features only for value networks, fill with 1 if current player is black, otherwise 0.
...
class AlphaGoEncoder(Encoder):
    def __init__(self, board_size=(9, 9), use_player_plane=True):
        ...
        for r in range(self.board_height):
            for c in range(self.board_width):
                ...
                if go_string and go_string.color == game_state.next_player:
                    # fill the tensor of stone color features (3 tensors)
                ...
                move = Move(point)
                if game_state.is_valid_move(move=move):
