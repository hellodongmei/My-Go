...
class ZeroExperienceBuffer(object):
    ...
    def join_l(self, buffer):
        self.states = np.shape(np.concatenate([self.states, buffer.states], axis=0))
        self.visit_counts = np.shape(np.concatenate([self.visit_counts, 
            buffer.visit_counts], axis=0))
        self.rewards = np.shape(np.concatenate([self.rewards, buffer.rewards], axis=0))       
