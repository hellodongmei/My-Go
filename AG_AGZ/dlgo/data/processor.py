...
class GoDataProcessor:
    def __init__(self, encoder='ZeroEncoder', data_directory='/home/users/l/liudong1/scratch/AG_MCTS/data'):
...
# tag::read_sgf_files[]
    def read_sgf(self, data_list, data_file_name):
        ...
        # print(feature_shape)
        # print(type(feature_shape))
        features = np.zeros(feature_shape)
        labels = np.zeros((total_examples,))
        rewards = np.zeros((total_examples,))
        print('total data point:'+ str(total_examples))
        ...
        for filename in data_list:
            ...
            # to find the winner color
            # if 'RE' in sgf_content:
            #     win_color = sgf_content.split('RE')[1][1]
            # # else:
            # #     win_color = 'none'
            ...
            for item in sgf.main_sequence_iter():  # <5>
                ...
                if color is not None:
                    ...
                    if first_move_done and point is not None:
                        ...
                        if 'RE' in sgf_content:
                            win_color = sgf_content.split('RE')[1][1]
                            if color == win_color.lower(): # encode the winner move's reward as 1
                                rewards[counter] = 1
                        # elif win_color = 'none':
                        #     rewards[counter] = 0
                            else:
...
# tag::store_features_and_labels[]
        ...
        reward_file_base = self.data_dir + '/' + data_file_name + '_rewards_%d'
        ...
        while features.shape[0] >= chunksize:  # <1>
            ...
            reward_file = reward_file_base % chunk
            chunk += 1
            current_features, features = features[:chunksize], features[chunksize:]
            current_labels, labels = labels[:chunksize], labels[chunksize:]  # <2>
            current_rewards, rewards = rewards[:chunksize], rewards[chunksize:] 
            np.save(feature_file, current_features)
            np.save(label_file, current_labels)  # <3>
            np.save(reward_file, current_rewards)
