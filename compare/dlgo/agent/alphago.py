...
# tag::select_node[]
    def select_child(self):
        return max(self.children.items(),
                   ...
                   child[1].u_value)
    # max lambda, the lambda is an anonymous function, key is the function agument
    # here is to let all 'child' in 'children' to compute the function after lambda
# end::select_node[]
...
# tag::expand_children[]
    def expand_children(self, moves, probabilities):
        for move, prob in zip(moves, probabilities):
            ...
            # for all legal move, add the alphagonode instance to them
# end::expand_children[]
...
# <4> the utility has extra terms, c_u is defaulted 5
# end::update_values[]
...
class AlphaGoMCTS(Agent):
    def __init__(self, policy_agent, fast_policy_agent, value_agent,
                 lambda_value=0.5, num_simulations=5,
                 depth=3, rollout_limit=10):
...
# end::alphago_mcts_init[]
    ...
    def set_paras(self, num_simulations, depth, rollout_limit):
        self.num_simulations = num_simulations
        self.depth = depth
        self.rollout_limit = rollout_limit
...
# tag::alphago_mcts_rollout[]
    def select_move(self, game_state):
        if len(game_state.board._grid) > 60:
            game_result = scoring.compute_game_result(game_state)
            next_player = game_state.next_player
            if game_result.winner == next_player and game_result.winning_margin > 5:
                return Move.pass_turn()
        # node = self.root
        for simulation in range(self.num_simulations):  # <1>
            current_state = game_state
            node = self.root
            #print('node.vist:',node.visit_count)
            #print('1:',len(node.children))
            #print('1 self:',len(self.root.children))
            for depth in range(self.depth):  # <2>
                if not node.children:  # <3>
                    ...
                    #print('moves:',moves)
                    #print(node)
                    if moves[1].point is None: # the moves is not None even its point is None
                        self.root = AlphaGoNode() # to avoid the node.children is not 0 at the beginning
                        return Move.pass_turn()
                    ...
                    #print(node)
                    #print(node.parent)
                    #print('2:',len(node.children))
                   # print('2 self:',len(self.root.children))

                move, node = node.select_child()  # <5>
                #print(node.parent)

                #print(move.point)
                #print('3:',len(node.children))
               # print('3 self:',len(self.root.children))
            ...
            #print(node.parent)
            #print('node.vist:',node.visit_count)
            #print('self.vist:',self.root.visit_count)
           # self.root.update_values(weighted_value)
            #print('self.vist2:',self.root.visit_count)
...
# tag::alphago_mcts_selection[]
        #print('final node:',len(node.children))
        #print('final self:',len(self.root.children))
        move = max(self.root.children, key=lambda move:  # <1>
                   self.root.children.get(move).visit_count)  # <1>
        #move = max(node.children, key=lambda move:  # <1>
        #     node.children.get(move).visit_count)  # <1>
        #print(move.point)
